...
if [[ -n "${OPTIMIZATION_LEVEL:-}" ]]; then
  OPTIMIZATION_ARGS+=("--optimization-level" "${OPTIMIZATION_LEVEL}")
fi

PERFORMANCE_ARGS=()
if [[ -n "${PERFORMANCE_MODE:-}" ]]; then
  PERFORMANCE_ARGS+=("--performance-mode" "${PERFORMANCE_MODE}")
fi

GENERATION_CONFIG_ARGS=()
if [[ -n "${OVERRIDE_GENERATION_CONFIG:-}" ]]; then
  GENERATION_CONFIG_ARGS+=("--override-generation-config" "${OVERRIDE_GENERATION_CONFIG}")
else
  GENERATION_CONFIG_ARGS+=(--generation-config vllm)
fi
